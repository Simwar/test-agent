 ...
 *
 * Env vars (set in .env, not auto-injected by Astro):
...
export async function searchIssues(jql: string): Promise<JiraIssueSummary[]> {
  ...
  while (true) {
    const data = await jiraFetch<JiraSearchResponse>('/search', {
      jql,
    ...
    console.log(`  [jira] fetched ${allIssues.length}/${data.total} issues`);
