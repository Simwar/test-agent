...
export const searchJiraIssuesTool = createTool({
  ...
  inputSchema: z.object({
    projectKey: z.string().describe('Jira project key, e.g. "FLOWS"'),
  ...
  execute: async (input) => {
    try {
      const jql =
        `project = "${input.projectKey}" AND status changed to "Done" ` +
...
export const getJiraIssueDetailsTool = createTool({
  ...
  inputSchema: z.object({
    issueKey: z.string().describe('Jira issue key, e.g. "FLOWS-123"'),
  }),
  outputSchema: z.object({
    key: z.string(),
    summary: z.string(),
